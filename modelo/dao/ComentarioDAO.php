...
class ComentarioDAO{
    ...
    public function verComentariosXart($idArticulo){
        ...
        $data_table = $data_source->ejecutarConsulta('SELECT idComentario,(SELECT primerNombre FROM usuarios WHERE usuarios.idUsuario = comentario.idUsuario) as "idUsuario", idArticulo, comentario FROM comentario WHERE idArticulo = :idArticulo', array(':idArticulo' => $idArticulo));
